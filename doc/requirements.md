...
- 详情页展示同一块滑板的正反两面：正面是玩家生成的喷绘图面；背面是第二次 img2img 生成的新图，以时间胶囊游戏同款 AlterU 白色 α Logo 参考图为核心，设计目标是比正面更简洁、更品牌化，不需要和正面图案建立强呼应关系。背面上方再叠加 SVG 绘制的拟物轮架、轮子、螺丝孔和轴承细节：两组轮架分别位于 y=82px 和 y=258px，SVG 视图为 150px × 350px，左右轮子各伸出板身约 19px，轮子用横向圆角矩形模拟俯视圆柱体，包含高光、侧边暗线、投影、金属轴、底座、4 个螺丝和中心主螺母。轮子提供黑色、奶白、薄荷 3 个颜色版本，真实作品按作品 id 稳定分配，评审页同时展示 3 个版本。两面尺寸均为 112px × 350px，宽高比约 1:3.1，圆角 56px，阴影 `0 22px 50px rgba(0,0,0,0.55)`，边缘高光 `rgba(255,255,255,0.13)`；正面和背面图片都必须等比填充，不允许横向或纵向拉伸。
...
- 资产清单：`public/img/review-deck-sheet.jpg` 是评审源图；`public/img/review-decks/deck-00.jpg` 到 `deck-11.jpg` 是实际评审/demo 使用的 12 张独立竖向滑板图；`public/img/review-back.svg` 是旧作品和离线 demo 的背面 fallback；`public/poster.svg` 用于游戏封面；`public/img/aigram.svg` 用于水印。
...
- 背面生成总是调用 img2img：`ref_url` 固定为时间胶囊验证过的 `https://images.aiwaves.tech/bag-watermark/alteru_white_1024.png`，prompt 要求“AlterU Greek alpha logo as central hero mark, simpler and more restrained than the front artwork, premium skate brand identity, no board outline, no wheels, no trucks, no extra text”。
