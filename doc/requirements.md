...
- 详情页展示同一块滑板的正反两面：正面是玩家生成的喷绘图面；背面是第二次 img2img 生成的新图，以时间胶囊游戏同款 AlterU 白色 α Logo 参考图为核心，并根据正面的头像/基础板风格选择相近色彩、半调、喷漆、贴纸和刮痕语言。背面上方再叠加 SVG 绘制的轮架、轮子、螺丝孔和轴承细节。两面尺寸均为 112px × 350px，宽高比约 1:3.1，圆角 56px，阴影 `0 22px 50px rgba(0,0,0,0.55)`，边缘高光 `rgba(255,255,255,0.13)`；正面和背面图片都必须等比填充，不允许横向或纵向拉伸。
...
- 背面生成总是调用 img2img：`ref_url` 固定为时间胶囊验证过的 `https://images.aiwaves.tech/bag-watermark/alteru_white_1024.png`，prompt 要求“AlterU Greek alpha logo as central hero mark, coordinate palette and texture with the front deck, no board outline, no wheels, no trucks, no extra text”。
