...
- 中央墙面从 y=126px 到 y=548px，展示 7-12 块不带轮子的彩色背面喷绘图面，单块图卡尺寸 92px × 286px，宽高比约 1:3.1，圆角 46px，形成接近滑板 deck 的修长大圆端轮廓；墙面使用竖向滚动的三列瓦片墙，行距 126px，因此下一行遮住上一行 160px（超过单片高度 50%），横向错位控制在 2-8px，整体保持整齐。
- 详情页展示同一块滑板的正反两面：正面是黑底 AlterU α Logo 品牌面，来自第二次 img2img 生成，视觉比彩色背面更暗、更简洁，并在详情页中以约 92px × 288px 的较小比例展示，不加轮子；这张 logo 品牌图本身必须是铺满矩形的图案，不允许自带滑板圆角边框、内框、边缘描线或板身轮廓，因为最终边框和滑板外形由 CSS 遮罩负责。背面是玩家头像或基础 prompt 生成的全彩喷绘图面，在详情页中以 112px × 350px 展示并叠加 SVG 拟物轮架、轮子、螺丝孔和轴承细节。两组轮架分别位于 y=68px 和 y=282px，更贴近板头和板尾；SVG 视图为 150px × 350px，左右轮子各伸出板身约 15px，轮子为 24px × 34px 横向圆角矩形，模拟俯视圆柱体，避免轮子过大，包含高光、侧边暗线、投影、金属轴、底座、4 个螺丝和中心主螺母。轮子提供黑色、奶白、薄荷 3 个颜色版本，真实作品按作品 id 稳定分配，评审页同时展示 3 个版本。正面和背面图片都必须等比填充，不允许横向或纵向拉伸。
