...
- 详情页展示同一块滑板的正反两面：正面是玩家生成的喷绘图面，背面是 SVG 绘制的 grip tape、轮架、轮子、螺丝孔和轴承细节。两面尺寸均为 112px × 350px，宽高比约 1:3.1，圆角 56px，阴影 `0 22px 50px rgba(0,0,0,0.55)`，边缘高光 `rgba(255,255,255,0.13)`；正面图片必须等比填充，不允许横向或纵向拉伸。
